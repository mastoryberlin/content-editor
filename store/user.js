...
export const actions = {
  async queryProfile({ commit }) {
    const response = await this.$axios.$get('https://' + process.env.NUXT_ENV_PROC_URL + '/content-editor/user/profile')
    if (response.success) {
  ...
  async queryPrivileges({ commit }) {
    const response = await this.$axios.$get('https://' + process.env.NUXT_ENV_PROC_URL + '/content-editor/user/privileges')
    if (response.success) {
      commit('setPrivileges', response.privileges)
    }
  },
  async updateProfile({ commit }, newData) {
    const response = await this.$axios.$post('https://' + process.env.NUXT_ENV_PROC_URL + '/content-editor/user/profile', newData)
    if (response.success) {
